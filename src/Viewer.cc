...
if (im.channels() == 1)
        cv::cvtColor(im, toShow, cv::COLOR_GRAY2BGR);  // Make it 3-channel BGR
    else
        toShow = im.clone();
           // --- YOLO overlay ---
std::vector<Detection> detections = mpTracker->GetLatestDetections();

for (const auto& det : detections)
{
    cv::rectangle(toShow, det.box, cv::Scalar(0, 0, 250), 2); // Red box
    std::string label = det.label + " " + std::to_string(int(det.confidence * 100)) + "%";
    cv::putText(toShow, label, det.box.tl() + cv::Point(2, 15),
